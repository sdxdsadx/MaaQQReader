...
当前结果：**192 个单元测试全部通过**（25 个测试文件）。核心包 `qqreader/` 不依赖任何第三方库；滑动验证码求解器可选依赖 `opencv-python` / `numpy`，测试需要 `pytest`。
...
直播/浏览类广告识别到 `进入直播间` / `直播中` / `需要下滑` / `上滑或点击` 等时，改为每 **5 秒下滑一次**，直到剩余秒数 <= 2 或达到最大下滑次数后退出。
