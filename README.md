...
2026-09-10 QQR-33 回归结果：**206 个单元测试全部通过**。核心包 `qqreader/` 不依赖任何第三方库；滑动验证码求解器可选依赖 `opencv-python` / `numpy`，测试需要 `pytest`。
...
直播/浏览类广告识别到 `进入直播间` / `直播中` / `需要下滑` / `上滑或点击` 等时，改为每 **5 秒下滑一次**，达到最大下滑次数后退出。
广告页出现「我要加速 / 去加速 / 点击加速 / 加速观看」时会先点击以继续观看；
广告完成弹窗「恭喜获得奖励 / 恭喜完成任务」会按旧 pipeline 点击左上角关闭。
奖励页只有同时识别到广告卡标题（`看小视频领好礼`）时才定位观看按钮。按钮被悬浮窗挡住或只识别到“立”时先滑动移开卡片，再观察；定位失败不再补点固定坐标。按钮坐标只缓存当前帧，每轮返回奖励页后重置等待和滑动计数。直播页没有“广告”标签也可根据“进入直播间/上滑浏览获取奖励”识别，继续观看/领奖弹窗优先于背景滑动。
...
- **广告完整 12/12 与连续 3 天无人值守未验收**：QQR-27～32 的 Done 不代表 QQR-14 的整轮验收完成；本轮修复和实跑记录见 `docs/QQR-33-ad-fix.md`。
